...
RSpec.describe GetWindStats do
	...
	context 'with correct params' do
		...
		context 'with a bigger amount of records' do
			before{
				...
				for i in 1..10
					...
					WindAvg.create(speed: rand(1.0..40.0), gust: rand(1.0..40.0), direction: rand(0.0..360.0), registered_date: Time.now - i.hours)
				end
			}
      ...
      it 'matches current log hour with time' do
				expect(response[:current][:registered_date].hour).to eq Time.zone.now.hour
      end
